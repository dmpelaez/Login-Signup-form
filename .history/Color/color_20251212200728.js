...
savedColor.push(...data.result);
 let colorGenerate = data.result.map(color => {
  ...
  return `
  <div class="generateColor" style="background-color: ${hex};" data-hex="${hex}">${hex}</div>`;
 }).join("");
...
btn.addEventListener('click', () => {
 ...
 icon.classList.toggle('fas fa-angle-up', isUp);
 icon.classList.toggle('fas fa-angle-down', !isUp);
